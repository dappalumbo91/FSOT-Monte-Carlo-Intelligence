...
# context → keywords: data/lang_tables/_shared.json via lang_tables.domain_keywords()
_FALLBACK_DOMAIN: Dict[str, Sequence[str]] = {
    "mythological": (
        "god", "goddess", "spirit", "myth", "hero", "underworld", "hades",
        "divine", "oracle", "sacrifice", "magic", "witchcraft", "sundial",
    ),
    "historical": (
        "king", "war", "law", "people", "city", "army", "hand", "water", "land",
    ),
    "linguistic": ("word", "speech", "language", "voice", "name", "write", "read"),
    "english": ("the", "a", "of", "to", "and"),
    "administrative": ("law", "record", "tax", "office", "order", "public"),
}


def _DOMAIN_KEYS() -> Dict[str, Sequence[str]]:
    try:
        from lang_tables import domain_keywords
...
def rank_senses(
    ...
    dk = _DOMAIN_KEYS()
    keys = dk.get(context, dk.get("historical", ()))
